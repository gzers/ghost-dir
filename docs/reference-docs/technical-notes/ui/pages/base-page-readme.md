...
#### `get_content_layout() -> QVBoxLayout`
获取内容区域布局，用于直接操作布局。
...
- `before_stretch=True`: 插入到底部 stretch 之前（推荐）
...
1. **设置视图（SettingView）**：由于直接继承 `ScrollArea` 且内容在滚动区域内，不适合使用 `BasePageView`，保持原有结构即可。

2. **帮助视图（HelpView）**：内容简单且不需要滚动，可使用 `enable_scroll=False` 或不使用基类。

3. **布局操作**：推荐使用 `add_to_content(widget)` 而不是直接操作 `get_content_layout()`，以确保控件正确添加到 stretch 之前。

4. **主题样式**：`BasePageView` 只处理容器背景色，自定义控件仍需自行处理主题样式。
