# Ghost-Dir 项目文档

欢迎来到 Ghost-Dir 项目文档中心！

## 📚 文档导航

### 📖 用户指南 ([guides/](./guides/))

新用户和日常使用指南：

- **[安装指南](./guides/INSTALL.md)** - 安装和环境配置
- **[配置指南](./guides/CONFIGURATION_GUIDE.md)** - 配置文件和选项说明
- **[文本层级指南](./guides/TEXT_HIERARCHY.md)** - 文本样式和层级系统
- **[快速参考](./guides/TEXT_QUICK_REFERENCE.md)** - 常用功能快速查询

---

### 🎯 实施计划 ([planning/](./planning/))

功能开发和重构计划：

- **[模板库 UI 优化计划](./planning/LIBRARY_UI_OPTIMIZATION_PLAN.md)** - Phase 5 UI 优化
- **[扫描进度重构计划](./planning/SCAN_PROGRESS_REFACTOR.md)** - 扫描进度组件重构

---

### 📊 实施报告 ([reports/](./reports/))

已完成工作的总结报告：

- **[扫描进度重构报告](./reports/SCAN_PROGRESS_REFACTOR_REPORT.md)** - 重构实施详情
- **[扫描进度对比](./reports/SCAN_PROGRESS_COMPARISON.md)** - 重构前后对比
- **[扫描进度总结](./reports/SCAN_PROGRESS_SUMMARY.md)** - 功能总结

---

### 🏗️ 架构文档 ([architecture/](./architecture/))

系统架构和设计文档：

- **[组件架构](./architecture/COMPONENT_ARCHITECTURE.md)** - 系统组件结构和关系

---

### 📁 其他文档目录

- **[planning/](./planning/)** - 详细规划文档
- **[progress/](./progress/)** - 进度跟踪文档
- **[tech_notes/](./tech_notes/)** - 技术笔记
- **[testing/](./testing/)** - 测试文档

---

## 🚀 快速开始

1. **新用户**：从 [安装指南](./guides/INSTALL.md) 开始
2. **配置系统**：查看 [配置指南](./guides/CONFIGURATION_GUIDE.md)
3. **了解架构**：阅读 [组件架构](./architecture/COMPONENT_ARCHITECTURE.md)
4. **查看计划**：浏览 [实施计划](./planning/) 了解未来功能

---

## 📝 文档贡献

如需添加或更新文档，请遵循以下结构：

- **guides/** - 用户指南和教程
- **planning/** - 功能计划和设计文档
- **reports/** - 实施报告和总结
- **architecture/** - 架构和设计文档

---

## 🔗 相关链接

- [项目主 README](../README.md)
- [源代码](../src/)
- [配置文件](../config/)

---

**最后更新**: 2026-01-26
