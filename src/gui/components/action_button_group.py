...
class ActionButtonGroup(QWidget):
    ...
    def _init_ui(self):
        ...
        if self.status == LinkStatus.DISCONNECTED:
            # 未连接:显示"建立连接"按钮
            establish_btn = PushButton(FluentIcon.LINK, t("console.establish"))
            ...
            # 已连接:显示"断开连接"按钮
            disconnect_btn = PushButton(FluentIcon.DISCONNECT, t("console.disconnect"))
            disconnect_btn.clicked.connect(self.disconnect_clicked.emit)
            layout.addWidget(disconnect_btn)
            
        elif self.status == LinkStatus.READY:
            # 就绪:显示"重新连接"按钮
            reconnect_btn = PushButton(FluentIcon.SYNC, t("console.reconnect"))
